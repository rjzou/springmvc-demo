					...
					<div class="widget-content">
						...
						<form role="form" method="post" action="${ctx}/waterelectcfg_edit"
							...
							<div class="form-group">
								<label class="control-label" for="inputWaterPrice">水价格(原水价格
									${params.waterprice}元)</label> <input type="number"
									class="form-control" id="inputWaterPrice"
									name="inputWaterPrice" value="${params.inputWaterPrice}"
									placeholder="输入新的水价格" required min="0" step="0.1">
							</div>

							<div class="form-group">
								<label class="control-label" for="inputElectPrice">电价格(原电价格
									${params.electprice}元)</label> <input type="number"
									class="form-control" id="inputElectPrice"
									name="inputElectPrice" value="${params.inputElectPrice}"
									placeholder="输入新的电价格" required min="0" step="0.1">
							</div>
							<input type="hidden" name="${_csrf.parameterName}"
								value="${_csrf.token}" />
							<button type="submit" class="btn btn-primary">保存</button>
							<a class="btn btn-warning" href="${ctx}/waterelectcfg"
								role="button">返回</a>
						</form>
					</div>
