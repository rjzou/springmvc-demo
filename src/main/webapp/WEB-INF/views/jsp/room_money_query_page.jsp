								...
								<c:if test="${params.times > 1}">
									<tr>
										...
										<td><c:choose>
												...
												<c:otherwise>
				        (${params.water}-${params.pre_water})*${params.waterprice} = <span
														class="label label-success">${params.usedwaterprice}</span>
												</c:otherwise>
											</c:choose></td>
									</tr>
									<tr>
										...
										<td><c:choose>
												...
												<c:otherwise>
				        (${params.elect}-${params.pre_elect})*${params.electprice} = <span
														class="label label-success">${params.usedelectprice}</span>
												</c:otherwise>
											</c:choose></td>
									</tr>
								</c:if>
								...
								<c:if test="${params.times == 1}">
									<tr>
										...
										<td>${params.keycount}个 * ${params.keyprice} 元/个 = <span
											class="label label-success">${params.sumkeyprice}</span></td>
									</tr>
								</c:if>
								<tr>
									<th class="col-sm-2">总收费用(房租 <c:if
											test="${params.times == 1}">  
            +押金
             </c:if> +水费+电费+网费+卫生费 <c:if test="${params.times == 1}">  
            +钥匙押金
             </c:if> )
									</th>
									<td><c:choose>
	            ...
	            +<span class="label label-success">${params.sumkeyprice}</span>
	             ...
	             = <h3>${params.msg}<span class="label label-warning">${params.roommoney}
														元</span>
												</h3>
											</c:otherwise>
										</c:choose></td>
							...
							<input type="hidden" id="houseid" name="houseid"
								...
								id="paymoney" name="paymoney" value="${params.paymoney}" /> <a
								class="btn btn-warning" href="${ctx}/room_money_query"
								role="button">返回</a>
						</form>
					</div>
