...
<body>
				...
				<div class="widget-content">
					...
					<div class="widget-content">
						...
						<form role="form" id="form1" method="post"
							action="${ctx}/room_checkout_add" data-toggle="validator">
							...
							<table class="table table-bordered">
								...
								<tr>
									...
									<td><span class="label label-success">${params.in_days}</span>
										天</td>
								</tr>
								<tr>
									...
									<td><c:choose>
											...
											<c:otherwise>
												...
												<div class="col-xs-4 form-group">
													<input type="number" class="form-control" id="curwater"
														name="curwater" class="form-control  input-sm"
														value="${params.curwater}" min="0" step="1"
														pattern="^[0-9]{1,}$" maxlength="10" required
														oninput="inputChange();" />
												</div>
												<strong>-${params.water} * ${params.waterprice} = </strong>
								...
								<tr>
									...
									<td><c:choose>
											...
											<c:otherwise>
												<div class="col-xs-4"form-group">
													<input type="number" class="form-control" id="curelect"
														class="form-control  input-sm" name="curelect"
														value="${params.curelect}" min="0" step="1"
														pattern="^[0-9]{1,}$" maxlength="10" required
														oninput="inputChange();" />
												</div>
												<strong> -${params.elect} * ${params.electprice} =
												</strong>
								...
								<tr>
									...
									<td>标配 ${params.keycount} 个 * ${params.keyprice} 元/个 = <span
										class="label label-success">${params.sumkeyprice}</span>
									</td>
								</tr>
								<tr>
									...
									<td>
										...
										<div class="col-xs-4"form-group">
											<input type="number" class="form-control" id="keycount"
												name="keycount" class="form-control  input-sm"
												value="${params.keycount}" min="0" step="1"
												pattern="^[0-9]{1,}$" maxlength="10" required
												oninput="inputChange();" />
										</div>
									</td>
								...
								<tr>
									...
									<td><c:choose>
											...
											<c:otherwise>
												<span class="label label-success">${params.pressmoney}</span> - <span
													class="label label-danger" id="lusedwaterprice">${params.usedwaterprice}</span>
	             - <span class="label label-danger" id="lusedelectprice">${params.usedelectprice}</span> - <span
													class="label label-danger">${params.netprice}</span>
	             - <span class="label label-danger">${params.trash}</span> - <span
													class="label label-danger" id="lkeyprice">0.0</span> 
	             = <h4>${params.msg}<span class="label label-warning"
														id="lcoutmoney">${params.backpressmoney}</span> 元
												</h4>
												<h4>
													房租退还<span class="label label-warning">${params.backmonthmoney}</span>
													元
												</h4>
												<h3>
													共计需退还<span class="label label-warning" id="lsumbackmoney">${params.sumbackmoney}</span>
													元
												</h3>
											</c:otherwise>
										</c:choose></td>
							...
							<input type="hidden" id="houseid" name="houseid"
								...
								id="sumbackmoney" name="sumbackmoney"
								value="${params.sumbackmoney}" /> <input type="hidden"
								name="${_csrf.parameterName}" value="${_csrf.token}" />
							<button type="submit" class="btn btn-primary"
								data-toggle="confirmation" data-placement="top"
								<c:if test="${!empty danger}">disabled</c:if>>确认退房</button>
							<a class="btn btn-warning" href="${ctx}/room_checkout"
								role="button">返回</a>
						</form>
						<br/>
						<p>
	...
	<script type="text/javascript">
		...
		function inputChange() {
			...
			$("#lusedwaterprice,#spanusedwaterprice").text(
					v_usedwaterprice.toFixed(1));
			$("#lusedelectprice,#spanusedelectprice").text(
					v_usedelectprice.toFixed(1));
			$("#lkeyprice").text(v_loseprice.toFixed(1));
			var v_coutmoney = (v_pressmoney - v_usedwaterprice
					- v_usedelectprice - v_netprice - v_trash - v_loseprice)
					.toFixed(1);
			$("#lcoutmoney").text(v_coutmoney);
			var v_backmonthmoney = parseFloat('${params.backmonthmoney}');
			var v_sumbackmoney = (parseFloat(v_coutmoney) + v_backmonthmoney)
					.toFixed(1)
